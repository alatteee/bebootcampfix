...
const PesertaController = {
	...
	createParticipant: async (req, res) => {
		try {
			...
			const imageUrl = `/settings/default-profile-image/${imageFileName}`
	...
	gradingParticipant: async (req, res) => {
		...
		try {
			...
			if (
				...
				const separatedJsonStrings =
					cleanedString.split(/(?<=})\s*,\s*(?={)/)
				...
				return res
					.status(200)
					.json({ msg: 'Penilaian berhasil diupdate' })
			} else if (
				penilaian &&
	...
	updateParticipant: async (req, res) => {
		...
		try {
			...
			const updatedParticipant = await existingParticipant.update({
				...
				nama_peserta:
					req.body.nama_peserta || existingParticipant.nama_peserta,
				jenis_kelamin:
					req.body.jenis_kelamin || existingParticipant.jenis_kelamin,
				nomor_handphone:
					req.body.nomor_handphone ||
					existingParticipant.nomor_handphone,
				alamat_rumah:
					req.body.alamat_rumah || existingParticipant.alamat_rumah,
				email: req.body.email || existingParticipant.email,
				tanggal_lahir:
					req.body.tanggal_lahir || existingParticipant.tanggal_lahir,
				link_github:
					req.body.link_github || existingParticipant.link_github,
			...
			try {
				if (
					...
					console.log(
						'Participant ID:',
						existingParticipant.peserta_id
					)
	...
	changePassword: async (req, res) => {
		...
		try {
			...
			if (newPassword.toLowerCase() === 'bootcamp') {
				return res.status(400).json({
					msg: "Cannot change password to default 'bootcamp'",
				})
			}
			...
			if (existingParticipant.password_peserta === 'bootcamp') {
				return res.status(400).json({
					msg: "Cannot change password from default 'bootcamp'",
				})
			}
